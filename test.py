...
class InterpreterTest(unittest.TestCase):
    def test_huffman_tree(self):
        self.assertEqual(
            Encoding.build_huffman_tree({"a": 10, "b": 2}), 
                                        [12, ["b", "0"], ["a", "1"]]
        )
    
    def test_encoding_file(self):
        self.assertEqual(
            Encoding.start('test.txt'), '1101110010'
        )
    ...
    def test_encodeing(self):
        self.assertEqual(
            open("text2_encode.txt").read(), "1111111111010010001101100111000000010000101111"
        )
    
    def test_file(self):
        self.assertEqual(
            open("test.txt").read(), "Hello"
        )
